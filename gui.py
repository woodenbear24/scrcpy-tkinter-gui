...
def create_interface():
    ...
    list_top_button = ttk.Button(left_frame, text="Refresh", command=lambda: Refresh_List(entry0, listbox))
    list_top_button.pack(side=tk.TOP, fill=tk.X, padx=5, pady=5)  

    # Create listbox with scrollbar
    list_scrollbar = ttk.Scrollbar(left_frame, orient=tk.VERTICAL)
    ...
    label1 = ttk.Label(right_frame, text="Device:")
    ...
    # First input field
    label1 = ttk.Label(right_frame, text="App Name:")
    label1.grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
    entry1 = ttk.Entry(right_frame)
    entry1.grid(row=1, column=1, sticky=tk.EW, padx=5, pady=5)

    # Second input field
    label2 = ttk.Label(right_frame, text="Bitrate(M):")
    ...
    # Third input field
    label3 = ttk.Label(right_frame, text="Video Codec:")
    ...
    # Fourth input field
    label4 = ttk.Label(right_frame, text="Other Args:")
    label4.grid(row=4, column=0, sticky=tk.W, padx=5, pady=5)
    entry4 = ttk.Entry(right_frame)
    entry4.grid(row=4, column=1, sticky=tk.EW, padx=5, pady=5)

    # Button
    button0 = ttk.Button(right_frame, text="Load Setting", command=lambda: Setting(entry2, entry3, entry4, 0))
    button0.grid(row=5, column=0, sticky=tk.EW, columnspan=2, padx=20, pady=1)
    button1 = ttk.Button(right_frame, text="Save Setting", command=lambda: Setting(entry2, entry3, entry4, 1))
    button1.grid(row=6, column=0, sticky=tk.EW, columnspan=2, padx=20, pady=1)
    button2 = ttk.Button(right_frame, text="Execute", command=lambda: cmd_generate(entry0, entry1, entry2, entry3, entry4))
